...
// TODO change
const dataPath = path.join(sysRoot, '.damsdevlauncher')
 ...
 * @returns {boolean} Whether or not the game will launch as a detached process.
 */
 exports.getBackgroundRespawn = function(def = false){
    return !def ? config.settings.game.backgroundRespawn : DEFAULT_CONFIG.settings.game.backgroundRespawn
}

/**
 * Change the status of whether or not the game should launch as a detached process.
 * 
 * @param {boolean} backgroundRespawn Whether or not the game should launch as a detached process.
 */
exports.setBackgroundRespawn = function(backgroundRespawn){
    config.settings.game.backgroundRespawn = backgroundRespawn
}
